...
if testing==True:    
    torch.save(network_best.state_dict(), 'training_data_%s_//' %opt_preprocess + opt + '_N%d' %N + '_%.3d' %idx + '_weight')
    np.save('training_data_%s_//' %opt_preprocess + opt + '_N%d' %N + '_%.3d' %idx + '_info', dict_info)
else:   
    torch.save(network_best.state_dict(), '/scratch/cfoliveiradasi/railway_largescale/training_data_%s_/' %opt_preprocess + opt + '_N%d' %N + '_%.3d' %idx + '_weight')
    np.save('/scratch/cfoliveiradasi/railway_largescale/training_data_%s_/' %opt_preprocess + opt + '_N%d' %N + '_%.3d' %idx + '_info', dict_info)
