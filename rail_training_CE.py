...
class Network_mask1(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, lr, n_actions, batch_size, dropout, list_masks, device, batch_first=True):
        ...
        self.batch_size = batch_size
        self.batch_first = batch_first
        self.network_type = 'Network_mask1'
...
class Network_mask2(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, lr, n_actions, batch_size, dropout, list_masks, device, batch_first=True):
        ...
        self.batch_size = batch_size
        self.batch_first = batch_first
        self.network_type = 'Network_mask2'
...
def build_stacked_label(batch_list_actions,num_actions, N_control):
    ...
    for i in range(batch_size):
        tmp_vector = build_label(batch_list_actions[i],num_actions, N_control)
...
def build_pad_batch_states_tensor(batch_states, batch_size, input_size, N_control):
    ...
    pad_batch_states = torch.cat((batch_states,b), dim=1)
...
def train_network(network, output_get_preprocessed_data, time_training, N_control, LR_schedule, device):
    ...
    if network.network_type == 'Network_CE':
        list_CE_weights = []
        list_CE_masks = []
    ...
    # for i in range(N_iter):
        ...
        if network.network_type == 'Network_CE':
            ...
            loss = 0
            for j in range(N_control):
                loss += nn.CrossEntropyLoss(weight=torch.tensor(list_CE_weights[j]))(net_eval[:, j, list_CE_masks[j]], labels_batch_train[:, j, list_CE_masks[j]])
            network.zero_grad()
            loss.backward()
        ...
        with torch.no_grad():
            ...
            if network.network_type == 'Network_CE':
                ...
                loss = 0
                for j in range(N_control):
                    loss += nn.CrossEntropyLoss(weight=torch.tensor(list_CE_weights[j]))(net_eval[:, j, list_CE_masks[j]], labels_batch_val[:, j, list_CE_masks[j]]).item()
